...
PM_START_TEXT = """
*Hᴇʟʟᴏ {} * [!](https://telegra.ph/file/0125e8e8815bd3d495029.jpg)
───────────────────────
...
Hᴇʀᴇ  Aʀᴇ  Sᴏᴍᴇ  Essᴇɴᴛɪᴀʟᴛ  Tᴏ  Tʀʏ  Oɴ Gojo.
...
*Note*: Gojo Music Bot works on a single merged commands for Music and Video
...
✗ *Gojo Database Saved Playlists*:

/createplaylist
- Create Your Playlist on Gojo's Server with Custom Name
...
/playplaylist 
- Start playing Your Saved Playlist on Gojo Servers.""",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
...
Gojo has a additional feature for non-admin users who want to use admin commands
...
/ping
- Ping the Bot and check Ram, Cpu etc stats of Gojo.""",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="🔙 𝘽𝙖𝙘𝙠", callback_data="tiana_music")]]
            ),
...
• We also thank all the groups who rely on our Bot for this service, we hope you will always like it: we are constantly working to improve it!""",
        ...
        query.message.edit_text(
            text="""*Gojobot is Now Open Source Bot Project.*
...
*Click below Button to Get Source Code.*""",
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="📄 𝙎𝙤𝙪𝙧𝙘𝙚", url="github.com"),                 
                    InlineKeyboardButton(text="🔙 𝘽𝙖𝙘𝙠", callback_data="about_"),
                 ]    
                ]
            ),
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "GojoB𝙤𝙩 𝙐𝙥𝙙𝙖𝙩𝙚𝙙 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮✅")
        except Unauthorized:
            LOGGER.warning(
